...
try {
    ...
    if (isset($_GET['id'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM TipoSangre WHERE IDTipoSangre = ?");
        $stmt->execute([$id]);
        $tipoSangre = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$tipoSangre) {
            die("Tipo de Sangre no encontrado.");
        }
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $conn->prepare("UPDATE TipoSangre SET NombreTipoSangre = ? WHERE IDTipoSangre = ?");
...
<body>
    <div class="container">
        ...
        <form method="POST">
            <input type="hidden" name="id" value="<?= htmlspecialchars($tipoSangre['IDTipoSangre']) ?>">
            <div class="form-group">
                <label for="nombre">Nombre del Tipo de Sangre</label>
                <input type="text" id="nombre" name="nombre" value="<?= htmlspecialchars($tipoSangre['NombreTipoSangre']) ?>" required>
            </div>
            <div class="form-group">
