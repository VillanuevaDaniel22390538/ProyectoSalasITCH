...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["buscar"])) {
        ...
        // Obtener información médica del encargado
        $stmt = $pdo->prepare("SELECT * FROM InformacionMedicaEncargadoLaboratorio WHERE IDEncargadoLaboratorio = ?");
        $stmt->execute([$IDEncargadoLaboratorio]);
        $informacionMedica = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if (!$informacionMedica) {
            $mensaje = "No se encontró información médica para el encargado.";
        }
    } elseif (isset($_POST["editar"])) {
        $IDInformacionMedica_EL = $_POST["IDInformacionMedica_EL"];
        $IDAlergias = $_POST["IDAlergias"];
        $IDEnfermedadesCronicas = $_POST["IDEnfermedadesCronicas"];
        $IDTipoSangre = $_POST["IDTipoSangre"];

        // Actualizar información médica
        $stmt = $pdo->prepare("UPDATE InformacionMedicaEncargadoLaboratorio SET IDAlergias = ?, IDEnfermedadesCronicas = ?, IDTipoSangre = ? WHERE IDInformacionMedica_EL = ?");
        $stmt->execute([$IDAlergias, $IDEnfermedadesCronicas, $IDTipoSangre, $IDInformacionMedica_EL]);
        $mensaje = "Información médica actualizada exitosamente.";
    } elseif (isset($_POST["borrar"])) {
        $IDInformacionMedica_EL = $_POST["IDInformacionMedica_EL"];

        // Borrar información médica
        $stmt = $pdo->prepare("DELETE FROM InformacionMedicaEncargadoLaboratorio WHERE IDInformacionMedica_EL = ?");
        $stmt->execute([$IDInformacionMedica_EL]);
...
<head>
    ...
    <title>Información Médica Encargado de Laboratorio</title>
    <style>
        body {
...
<a href="javascript:history.back()" class="back-arrow">&#x2190; </a>
    ...
    <div class="container">
        <div class="title">Información Médica Encargado de Laboratorio</div>
        <form method="POST">
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($informacionMedica as $registro): ?>
                        <tr>
                            <form method="POST">
                                <td><?= htmlspecialchars($registro['IDInformacionMedica_EL']) ?></td>
                                <td>
                                ...
                                <td>
                                    <input type="hidden" name="IDInformacionMedica_EL" value="<?= htmlspecialchars($registro['IDInformacionMedica_EL']) ?>">
