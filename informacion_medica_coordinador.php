...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["buscar"])) {
        ...
        // Obtener información médica del coordinador
        $stmt = $pdo->prepare("SELECT * FROM InformacionMedicaCoordinador WHERE IDCoordinador = ?");
        $stmt->execute([$IDCoordinador]);
        $informacionMedica = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if (!$informacionMedica) {
            $mensaje = "No se encontró información médica para el coordinador.";
        }
    } elseif (isset($_POST["editar"])) {
        $IDInformacionMedica_COO = $_POST["IDInformacionMedica_COO"];
        $IDAlergias = $_POST["IDAlergias"];
        $IDEnfermedadesCronicas = $_POST["IDEnfermedadesCronicas"];
        $IDTipoSangre = $_POST["IDTipoSangre"];

        // Actualizar información médica
        $stmt = $pdo->prepare("UPDATE InformacionMedicaCoordinador SET IDAlergias = ?, IDEnfermedadesCronicas = ?, IDTipoSangre = ? WHERE IDInformacionMedica_COO = ?");
        $stmt->execute([$IDAlergias, $IDEnfermedadesCronicas, $IDTipoSangre, $IDInformacionMedica_COO]);
        $mensaje = "Información médica actualizada exitosamente.";
    } elseif (isset($_POST["borrar"])) {
        $IDInformacionMedica_COO = $_POST["IDInformacionMedica_COO"];

        // Borrar información médica
        $stmt = $pdo->prepare("DELETE FROM InformacionMedicaCoordinador WHERE IDInformacionMedica_COO = ?");
        $stmt->execute([$IDInformacionMedica_COO]);
...
<head>
    ...
    <title>Información Médica Coordinador</title>
    <style>
        body {
...
<a href="javascript:history.back()" class="back-arrow">&#x2190; </a>
    ...
    <div class="container">
        <div class="title">Información Médica Coordinador</div>
        <form method="POST">
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($informacionMedica as $registro): ?>
                        <tr>
                            <form method="POST">
                                <td><?= htmlspecialchars($registro['IDInformacionMedica_COO']) ?></td>
                                <td>
                                ...
                                <td>
                                    <input type="hidden" name="IDInformacionMedica_COO" value="<?= htmlspecialchars($registro['IDInformacionMedica_COO']) ?>">
