...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["buscar"])) {
        ...
        // Obtener información médica del personal
        $stmt = $pdo->prepare("SELECT * FROM InformacionMedicaPersonal WHERE IDPersonal = ?");
        $stmt->execute([$IDPersonal]);
        $informacionMedica = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if (!$informacionMedica) {
            $mensaje = "No se encontró información médica para el personal.";
        }
    } elseif (isset($_POST["editar"])) {
        $IDInformacionMedica_PER = $_POST["IDInformacionMedica_PER"];
        $IDAlergias = $_POST["IDAlergias"];
        $IDEnfermedadesCronicas = $_POST["IDEnfermedadesCronicas"];
        $IDTipoSangre = $_POST["IDTipoSangre"];

        // Actualizar información médica
        $stmt = $pdo->prepare("UPDATE InformacionMedicaPersonal SET IDAlergias = ?, IDEnfermedadesCronicas = ?, IDTipoSangre = ? WHERE IDInformacionMedica_PER = ?");
        $stmt->execute([$IDAlergias, $IDEnfermedadesCronicas, $IDTipoSangre, $IDInformacionMedica_PER]);
        $mensaje = "Información médica actualizada exitosamente.";
    } elseif (isset($_POST["borrar"])) {
        $IDInformacionMedica_PER = $_POST["IDInformacionMedica_PER"];

        // Borrar información médica
        $stmt = $pdo->prepare("DELETE FROM InformacionMedicaPersonal WHERE IDInformacionMedica_PER = ?");
        $stmt->execute([$IDInformacionMedica_PER]);
...
<head>
    ...
    <title>Información Médica Personal de Laboratorio</title>
    <style>
        body {
...
<a href="javascript:history.back()" class="back-arrow">&#x2190; </a>
    ...
    <div class="container">
        <div class="title">Información Médica Personal de Laboratorio</div>
        <form method="POST">
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($informacionMedica as $registro): ?>
                        <tr>
                            <form method="POST">
                                <td><?= htmlspecialchars($registro['IDInformacionMedica_PER']) ?></td>
                                <td>
                                ...
                                <td>
                                    <input type="hidden" name="IDInformacionMedica_PER" value="<?= htmlspecialchars($registro['IDInformacionMedica_PER']) ?>">
